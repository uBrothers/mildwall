...
console.log("DB connected to optimize!");
var pressDone=0
  , next=0
  , nextTemp=0;
setInterval(function(){
  tempCoord.saveCoord.findOne({},function(err,optData){
    if(err){
      throw err;
    }
    if(optData){
      ...
      if(nextTemp==1){
        tempCoord.saveCoord.remove({x:opX,y:opY,xx:opXX,yy:opYY});
        nextTemp=0;
      }
    }
  });
...
setInterval(function(){
  if(global.gc){
    ...
    console.log("nodejs --expose-gc optimize.js");
  }
},30000);
